...
class BaseAliPay():
    ...
    def build_body(
        self, method, biz_content=None, return_url=None, notify_url=None, append_auth_token=False, **kwargs
    ):
        if biz_content:
            data = {
                "app_id": self._appid,
    ...
    def api_alipay_system_oauth_token(self, grant_type=None, code=None, refresh_token=None):
        assert grant_type in ("authorization_code", "refresh_token"), "unknown payee type"
        ...
        raw_string = urlopen(url, timeout=15).read().decode("utf-8")
        return self._verify_and_return_sync_response(
            raw_string, "alipay_system_oauth_token"
        )

    def api_alipay_user_info_share(self, auth_token=None, app_auth_token=None):
        if auth_token is None:
            raise Exception("auth_token is None!")
        ...
        raw_string = urlopen(url, timeout=15).read().decode("utf-8")
        return self._verify_and_return_sync_response(
            raw_string, "alipay_user_info_share"
        )
